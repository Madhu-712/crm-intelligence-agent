...
try:
    ...
    PUBSUB_LIB_AVAILABLE = True
except ImportError:
    PUBSUB_LIB_AVAILABLE = False
...
with st.sidebar:
    ...
    configured_pubsub_subscription = get_secret(
        "PUBSUB_SUBSCRIPTION",
        os.environ.get("PUBSUB_SUBSCRIPTION", "")
    )
    PUBSUB_SUBSCRIPTION = st.text_input(
        "Pub/Sub subscription",
        value=str(configured_pubsub_subscription).strip(),
        placeholder="projects/my-project/subscriptions/crm-inserts",
        help="Use the full subscription path or a subscription name in the configured GCP project.",
    ).strip()
    ...
    if PUBSUB_SUBSCRIPTION:
        st.caption(f"Pub/Sub subscription: `{PUBSUB_SUBSCRIPTION}`")
    else:
        st.caption("Pub/Sub is disabled. Set `PUBSUB_SUBSCRIPTION` to enable insert notifications.")
...
# ==============================================================================
# 4A. PUB/SUB BIGQUERY INSERT NOTIFICATION PIPELINE
# ==============================================================================
pubsub_events: Queue[Dict[str, Any]] = Queue()
pubsub_results: Queue[Dict[str, Any]] = Queue()
pubsub_seen_events: set[str] = set()
pubsub_subscriber_lock = threading.Lock()


def _decode_pubsub_event(message: Any) -> Optional[Dict[str, Any]]:
    """Decode a Pub/Sub message containing a BigQuery insert row or event envelope."""
    try:
        payload = json.loads(message.data.decode("utf-8"))
        if isinstance(payload, dict) and isinstance(payload.get("data"), dict):
            payload = payload["data"]
        row = payload.get("row", payload) if isinstance(payload, dict) else None
        ...
        return {
            ...
            "event_id": getattr(message, "message_id", "") or hashlib.sha256(
                message.data
            ).hexdigest(),
        }
...
def _process_pubsub_message(message: Any) -> None:
    ...
    with pubsub_subscriber_lock:
        if event_id in pubsub_seen_events:
            message.ack()
            return
        pubsub_seen_events.add(event_id)
        if len(pubsub_seen_events) > 1000:
            pubsub_seen_events.pop()

    pubsub_events.put(event)
    message.ack()


@st.cache_resource(show_spinner=False)
def start_pubsub_subscriber(subscription_name: str) -> Optional[Any]:
    """Start one subscriber per Streamlit process and return its streaming future."""
    if not PUBSUB_LIB_AVAILABLE or not subscription_name:
        return None
    subscriber = pubsub_v1.SubscriberClient()
    subscription_path = (
        subscription_name if subscription_name.startswith("projects/")
        else subscriber.subscription_path(GCP_PROJECT, subscription_name)
    )
    future = subscriber.subscribe(subscription_path, callback=_process_pubsub_message)
    # Keep the client alive for as long as the cached streaming future is used.
    return subscriber, future


def process_pending_pubsub_events() -> int:
    """Run the SQL and natural-language analysis for events received since the last rerun."""
    processed = 0
    while True:
        try:
            event = pubsub_events.get_nowait()
        except Empty:
            return processed

        row = event["row"]
        customer_id = row.get("CustomerID", row.get("customer_id"))
        if customer_id is not None:
            sql = f"SELECT * FROM {FULL_TABLE_PATH} WHERE CustomerID = {int(customer_id)} LIMIT 1"
        else:
            sql = f"SELECT * FROM {FULL_TABLE_PATH} ORDER BY SignupDate DESC LIMIT 1"

        sql_result = run_bigquery_sql(sql)
        request = event["natural_language_request"] or (
            "Analyze this newly inserted CRM lead, identify revenue or churn implications, "
            "and recommend the next sales or customer-success action."
        )
        analysis = execute_agent_chat(
            f"{request}\n\nNew BigQuery row:\n{json.dumps(row, default=str)}\n"
            f"SQL used to verify the row:\n{sql}\nSQL result:\n{sql_result}",
            [],
        )
        pubsub_results.put({
...
def poll_pubsub_events() -> None:
    """Refresh the app when the subscriber receives a new event."""
    if process_pending_pubsub_events() > 0:
        st.rerun()
...
{dataframe_to_markdown(churn_df[['CustomerID', 'FirstName', 'LastName', 'Email', 'TotalSpent', 'Notes']])}
...
{dataframe_to_markdown(top_df[['CustomerID', 'FirstName', 'LastName', 'Country', 'TotalSpent', 'LeadSource', 'Notes']])}
...
# Start the subscriber only when configured. The callback acknowledges messages
# quickly and the Streamlit thread performs the BigQuery/AI work on rerun.
if PUBSUB_SUBSCRIPTION:
    subscriber_handle = start_pubsub_subscriber(PUBSUB_SUBSCRIPTION)
else:
...
# ------------------------------------------------------------------------------
# TAB 5: PUB/SUB INSERT NOTIFICATIONS
# ------------------------------------------------------------------------------
with tabs[4]:
    st.subheader("🔔 BigQuery Insert Notifications")
    st.caption(
        "Publish a JSON message containing a `row` object to the configured subscription. "
        "Each message is verified with SQL and analyzed in natural language."
    )

    if not PUBSUB_LIB_AVAILABLE:
        st.warning(
            "The optional `google-cloud-pubsub` package is not installed. "
            "Install it and restart Streamlit to enable this feature."
        )
    elif not PUBSUB_SUBSCRIPTION:
        st.info("Set `PUBSUB_SUBSCRIPTION` (or a Streamlit secret) to start listening.")
    elif subscriber_handle is not None:
        st.success(f"Listening on `{PUBSUB_SUBSCRIPTION}`")
        st.code(
            '{"row": {"CustomerID": 1021, "FirstName": "New", "LastName": "Lead"}, '
            '"natural_language_request": "Summarize this new lead and suggest a follow-up."}',
            language="json",
        )
    ...
    while True:
        try:
            notification = pubsub_results.get_nowait()
        except Empty:
            break
        notification_count += 1
        st.toast("New BigQuery entry analyzed", icon="🔔")
        with st.expander(
            f"New entry received at {notification['received_at']}", expanded=True
        ):
            st.json(notification["row"])
            st.code(notification["sql"], language="sql")
            st.markdown("**SQL verification result**")
            st.code(notification["sql_result"])
            st.markdown("**Natural-language analysis**")
            st.markdown(notification["analysis"])

    if notification_count == 0:
        st.info("No new BigQuery insert notifications have been received in this session.")
